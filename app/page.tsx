...
export const metadata: Metadata = {
    ...
    description: 'Find board-certified pain management doctors near you. Filter by VA Community Care, specialty, and conditions treated. 15,000+ physicians nationwide.',
};
    ...
    return (<div>
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-offwhite">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {SPECIALTIES.map((s) => (<Link key={s.slug} href={`/listings?specialty=${s.slug}`} className="card p-5 flex items-center gap-3 group hover:border-navy-200">
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {CONDITIONS.map((c) => (<Link key={c.slug} href={`/chronic-pain-conditions/${c.slug}`} className="rounded-2xl bg-offwhite border border-gray-100 p-4 text-center hover:border-navy-200 hover:shadow-soft transition-all group">
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-offwhite">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {PROCEDURES.map((p) => (<Link key={p.slug} href={`/pain-treatments/${p.slug}`} className="card p-4 text-center group hover:border-teal-200">
      ...
      {/* Cities */}
      {topCities.length > 0 && (<section className="py-16 px-4 sm:px-6 lg:px-8 bg-offwhite">
          <div className="mx-auto max-w-7xl">
            <div className="text-center mb-10">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
              {topCities.map((city) => (<Link key={`${city.city}-${city.state}`} href={`/pain-management-doctors/${city.city.toLowerCase().replace(/\s+/g, '-')}-${city.state.toLowerCase()}`} className="rounded-xl bg-white px-3 py-3.5 text-center shadow-soft hover:shadow-card transition-shadow group">
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-navy">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <Link href="/listings?va=true" className="inline-flex items-center justify-center gap-2 rounded-full bg-white px-8 py-4 text-base font-semibold text-navy hover:bg-navy-50 transition-colors">
